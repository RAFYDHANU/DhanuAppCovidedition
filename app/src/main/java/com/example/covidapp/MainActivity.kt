        ...
        api.getALlNegara().enqueue(object : Callback<AllNegara>{
            override fun onResponse(call: Call<AllNegara>, response: Response<AllNegara>) {
                if(response.isSuccessful){
                    ...
                    txt_recofred_globe.text = formatter.format(getListDataCorona?.TotalConfirmed?.toDouble())
                    txt_deaths_globe.text = formatter.format(getListDataCorona?.TotalConfirmed?.toDouble())
                    rv_country.apply {
                        setHasFixedSize(true)
                        layoutManager = LinearLayoutManager(this@MainActivity)
